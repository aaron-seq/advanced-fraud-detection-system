...
class VisitorFingerprintEngine:
    ...
    def generate_fingerprint(self, request_context: dict[str, Any]) -> DeviceFingerprint:
        """
        ...
        try:
            ...
            raw_fingerprint = self.fingerprint_generator.create(
                device=device_attrs, network=network_attrs, behavioral=behavioral_attrs
            )
            ...
            return self._generate_fallback_fingerprint(e.partial_attributes, request_id)
    ...
    def _generate_fallback_fingerprint(
        self, partial_attributes: dict[str, Any], request_id: str | None
    ) -> DeviceFingerprint:
        """
        ...
        raw_fingerprint = self.fingerprint_generator.create(
            device=device_attrs, network=NetworkAttributes(), behavioral=BehavioralAttributes()
        )
    ...
    def match_fingerprint(
        self, fingerprint: DeviceFingerprint, known_fingerprints: list
        ...
        for known_fp in known_fingerprints:
            ...
            if fp_hierarchy.get("fine") and fp_hierarchy.get("fine") == known_hierarchy.get("fine"):
                return known_fp

            # Medium match with high confidence
            if (
                fp_hierarchy.get("medium")
                and fp_hierarchy.get("medium") == known_hierarchy.get("medium")
                and fingerprint.confidence >= 0.8
            ):
                return known_fp
