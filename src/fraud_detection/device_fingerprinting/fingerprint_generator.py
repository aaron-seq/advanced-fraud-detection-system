...
class StableFingerprintGenerator:
    ...
    def create(
        self, device: DeviceAttributes, network: NetworkAttributes, behavioral: BehavioralAttributes
    ) -> RawFingerprint:
        """
    ...
    def _generate_coarse_fingerprint(self, device: DeviceAttributes) -> dict[str, Any]:
        """
    ...
    def _generate_medium_fingerprint(
        self, device: DeviceAttributes, coarse: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    def _generate_fine_fingerprint(
        ...
        if device.gpu_renderer:
            gpu_hash = hashlib.sha256(device.gpu_renderer.encode()).hexdigest()[:8]
        ...
        if network.isp:
            isp_hash = hashlib.sha256(network.isp.encode()).hexdigest()[:8]
    ...
    def _calculate_stability_score(
        self, device: DeviceAttributes, network: NetworkAttributes
    ) -> float:
        """
    ...
    def _serialize_device_attrs(self, device: DeviceAttributes) -> dict[str, Any]:
        ...
        return {k: v for k, v in device.__dict__.items() if v not in (None, "", [], {})}
    ...
    def _serialize_behavioral_attrs(self, behavioral: BehavioralAttributes) -> dict[str, Any]:
        ...
        return {k: v for k, v in behavioral.__dict__.items() if v not in (None, "", [], {})}
