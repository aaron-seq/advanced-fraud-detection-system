...
from ...block.models import BehavioralAttributes, DeviceAttributes, NetworkAttributes
...
class AttributeCollectionError(Exception):
    ...
    def __init__(self, message: str, partial_attributes: dict[str, Any] | None = None):
...
class DeviceAttributeCollector:
    ...
    def collect(self, request_context: dict[str, Any]) -> DeviceAttributes:
        ...
        try:
            ...
            attrs = DeviceAttributes(
                ...
                webgl_fingerprint=self._hash_fingerprint(client_data.get("webgl_fingerprint", "")),
                audio_fingerprint=self._hash_fingerprint(client_data.get("audio_fingerprint", "")),
                fonts_hash=self._hash_fingerprint(str(client_data.get("fonts", []))),
                plugins_hash=self._hash_fingerprint(str(client_data.get("plugins", []))),
            )
    ...
    def _get_header(self, headers: dict[str, Any], name: str, default: str = "") -> str:
    ...
    def _parse_user_agent(self, user_agent: str) -> dict[str, str]:
        ...
        # Detect platform. Mobile is checked before desktop because mobile UAs
        # embed the desktop kernel they derive from: Android reports "Linux",
        # iOS reports "like Mac OS X". Checking desktop first misclassifies both.
...
class NetworkContextCollector:
    ...
    def collect(self, request_context: dict[str, Any]) -> NetworkAttributes:
        ...
        try:
            ...
            is_vpn, is_proxy, is_tor, is_datacenter = self._detect_anonymizers(ip_address, headers)
    ...
    def _get_client_ip(self, request_context: dict[str, Any], headers: dict[str, Any]) -> str:
        ...
        # Placeholder for "no client address available", not a bind address.
        return request_context.get("ip", "0.0.0.0")  # noqa: S104
    ...
    def _detect_anonymizers(
        self, ip_address: str, headers: dict[str, Any]
        ...
        proxy_headers = ["via", "x-proxy-id", "x-forwarded-for", "forwarded", "x-real-ip"]
        for key in headers:
    ...
    def collect(self, request_context: dict[str, Any]) -> BehavioralAttributes:
        ...
        try:
            ...
            attrs = BehavioralAttributes(
                ...
                interaction_pattern_hash=self._hash_interaction_pattern(behavioral_data),
            )
