...
from ...block.models import Transaction
...
class SpendingPatternAnalyzer:
    ...
    def build_pattern(
        self, user_id: str, transaction_history: list[dict[str, Any]]
    ) -> SpendingPattern:
        """
        ...
        hourly_dist = {hour: count / total_transactions for hour, count in hour_counts.items()}
        ...
        category_dist = {cat: count / total_transactions for cat, count in category_counts.items()}
        ...
        location_dist = {loc: count / total_transactions for loc, count in location_counts.items()}
        ...
        weekday_dist = {day: count / total_transactions for day, count in weekday_counts.items()}
    ...
    def detect_anomaly(self, transaction: Transaction, pattern: SpendingPattern) -> AnomalyScore:
        """
        ...
        # Amount anomaly
        amount_anomaly = self._detect_amount_anomaly(transaction.amount, pattern, factors)

        # Frequency anomaly
        frequency_anomaly = self._detect_frequency_anomaly(transaction, pattern, factors)

        # Time anomaly
        time_anomaly = self._detect_time_anomaly(transaction.timestamp, pattern, factors)

        # Category anomaly
        category_anomaly = self._detect_category_anomaly(
            transaction.merchant_category, pattern, factors
        )

        # Location anomaly
        location_anomaly = self._detect_location_anomaly(transaction.location, pattern, factors)

        # Velocity anomaly
        velocity_anomaly = self._detect_velocity_anomaly(transaction, pattern, factors)
    ...
    def _detect_amount_anomaly(
        self, amount: float, pattern: SpendingPattern, factors: list[str]
    ) -> float:
    ...
    def _detect_frequency_anomaly(
        self, transaction: Transaction, pattern: SpendingPattern, factors: list[str]
    ) -> float:
    ...
    def _detect_time_anomaly(
        self, timestamp: datetime, pattern: SpendingPattern, factors: list[str]
    ) -> float:
        ...
        # pattern.weekday_distribution is collected but not scored here, so an
        # otherwise-normal transaction on a day the user never transacts is
        # not flagged. Scoring it would need its own calibration.
    ...
    def _detect_category_anomaly(
        self, category: str, pattern: SpendingPattern, factors: list[str]
    ) -> float:
    ...
    def _detect_location_anomaly(
        self, location: dict[str, Any] | None, pattern: SpendingPattern, factors: list[str]
    ) -> float:
        ...
        if location_probability == 0:
            ...
            same_country = any(country in loc for loc in known_locations)
    ...
    def _parse_timestamp(self, value: Any) -> datetime | None:
        ...
        if isinstance(value, (int, float)):
            try:
                # Epoch seconds are UTC; without tz they would be read in the
                # server's local zone and shift the hour-of-day analysis.
    ...
    def analyze_transaction_batch(
        self, transactions: list[Transaction], pattern: SpendingPattern
