...
class BusinessMetricsCollector:
    ...
    def record_feedback(self, transaction_id: str, was_fraud: bool) -> None:
        """
    ...
    def get_period_stats(self, start: datetime, end: datetime) -> FraudPreventionStats:
        """
        ...
        """
        stats = FraudPreventionStats(period_start=start, period_end=end)

        # Filter decisions in period
        period_decisions = [d for d in self._decisions if start <= d["timestamp"] <= end]
    ...
    def get_model_performance(self, model_name: str) -> ModelPerformanceStats | None:
    ...
    def calculate_drift_score(self, model_name: str, window_hours: int = 24) -> float:
        """
        ...
        recent = [
            ...
            if start <= d["timestamp"] <= end and d["model_name"] == model_name
        ]
        ...
        # Only mean shift is measured. A distribution that keeps its mean but
        # widens is also drift; detecting that needs a variance test here.

        # Compare to baseline (all historical)
        all_model = [d for d in self._decisions if d["model_name"] == model_name]
    ...
    def _update_model_stats(self, model_name: str, latency_ms: float, decision: str) -> None:
        """Update model performance statistics."""
        if model_name not in self._model_stats:
            self._model_stats[model_name] = ModelPerformanceStats(
                model_name=model_name, model_version="unknown"
            )
        ...
        report = {
            ...
            "model_performance": {
                name: {
                    ...
                    "fraud_rate": (
                        s.fraud_predictions / s.total_predictions if s.total_predictions > 0 else 0
                    ),
