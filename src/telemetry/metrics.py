...
def setup_metrics(
    registry: Optional["CollectorRegistry"] = None, prefix: str = "fraud_detection"
) -> None:
    """
    ...
    # System info
    SYSTEM_INFO = Info(f"{prefix}_system", "Fraud detection system information", registry=_registry)
...
class FraudDetectionMetrics:
    ...
    def record_error(self, error_type: str, component: str) -> None:
        ...
        FRAUD_DETECTION_ERRORS.labels(error_type=error_type, component=component).inc()

    def record_decision(self, decision: str, risk_level: str) -> None:
        """Record a fraud detection decision."""
        if not self._enabled:
            return

        if FRAUD_DECISIONS:
            FRAUD_DECISIONS.labels(decision=decision, risk_level=risk_level).inc()

        if FRAUD_CHECKS_TOTAL:
            FRAUD_CHECKS_TOTAL.labels(status="completed", risk_level=risk_level).inc()
