...
# Default is None, not {}. A mutable default is shared by every context, so a
# single in-place write anywhere would leak one request's context into all
# others. Read it through get_request_context().
...
class HumanReadableFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        formatted = (
            ...
            f"{datetime.now(UTC).strftime('%H:%M:%S.%f')[:-3]} "
            f"{prefix}"
...
def set_request_context(context: dict[str, Any]) -> None:
    ...
    """
    request_context_var.set({**get_request_context(), **context})
...
class CorrelationIdMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        correlation_id = headers.get(b"x-correlation-id", headers.get(b"x-request-id"))
