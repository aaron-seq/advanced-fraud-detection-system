...
def parse_datetime(
    value: str | datetime | int | float, default: datetime | None = None
) -> datetime | None:
    """
    ...
    if isinstance(value, datetime):
        # Naive input is assumed to be UTC, matching the rest of the system.
    ...
    if isinstance(value, (int, float)):
        ...
        if value > 1e10:
            value = value / 1000
        try:
            # tz=UTC is required: epoch seconds are UTC by definition, and
            # without it Python interprets them in the server's local zone,
            # shifting every parsed time by the machine's offset.
            return datetime.fromtimestamp(value, tz=UTC)
        except (ValueError, OSError, OverflowError):
            return default

    if isinstance(value, str):
        # fromisoformat covers the full ISO 8601 grammar on Python 3.11+,
        # including "Z" and numeric offsets. The previous implementation
        # stripped the offset with a regex before parsing, so
        # "2026-02-03T11:30:00+05:30" was read as 11:30 UTC - a silent
        # five-and-a-half hour error on every offset-bearing timestamp.
        try:
        ...
        # Non-ISO legacy formats. Day-first is tried before month-first, so
        # genuinely ambiguous values like "01/02/2026" are read as 1 February.
...
def calculate_hash(data: str | bytes | dict, algorithm: str = "sha256") -> str:
    """
...
def validate_required_fields(
    data: dict[str, Any], required_fields: list[str], raise_on_missing: bool = True
) -> list[str]:
    """
...
def deep_set(data: dict[str, Any], path: str, value: Any, separator: str = ".") -> dict[str, Any]:
    """
...
def mask_sensitive_data(
    ...
    for key, value in data.items():
        if isinstance(value, dict):
            result[key] = mask_sensitive_data(value, sensitive_fields, mask_char, visible_chars)
        elif isinstance(value, str) and any(sf.lower() in key.lower() for sf in sensitive_fields):
...
def flatten_dict(
    data: dict[str, Any], separator: str = ".", parent_key: str = ""
) -> dict[str, Any]:
    """
