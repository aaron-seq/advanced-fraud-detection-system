...
from src.utils.error_handling import MLModelError
...
class TestFraudDetectionBlock:
    ...
    def test_detect_fraud_device_fingerprint_failure_fallback(self):
        ...
        deps["device_fingerprint_service"].generate.side_effect = Exception(
        ...
        assert (
            "fallback" in str(result.explanation).lower()
            or "limited" in str(result.explanation).lower()
        )
    ...
    def test_detect_fraud_ml_model_failure_fallback(self):
        ...
        assert (
            "rule" in str(result.explanation).lower()
            or "fallback" in str(result.explanation).lower()
        )
...
class TestFromErrorFallback:
    ...
    def test_from_error_creates_review_decision(self):
        ...
        result = FraudDetectionResult.from_error(
            error="Unexpected error occurred", request_id="req_error"
        )
